---
title: "HCAMP Baseline IMPACT Data"
output:
  html_document:
    toc: true
    toc_float: true
...
five_mem_verbal_outliers$is.extreme
```
...
three_mem_visual_long <- three_mem_visual %>% 
  pivot_longer(
    cols = c(5:7),
    names_to = "baseline_number",
    names_prefix = "user_memory_composite_score_",
...
```{r, include=TRUE}
box_p(three_mem_visual_long, baseline_number, score)
```
...
```{r, include=FALSE}
three_mem_visual_outliers <- three_mem_visual_long %>% 
  group_by(baseline_number) %>% 
  identify_outliers(score) %>% 
  select(-c(3:5))

three_mem_visual_outliers %>% 
  count(is.outlier)
...
```{r, include=TRUE, fig.width=10}
ggplot(three_mem_visual_long, aes(sample = score)) +
  stat_qq(alpha = 0.3) +
  stat_qq_line() +
  facet_wrap(~baseline_number)
```
...
```{r, include=FALSE}
three_mem_visual_res <- anova_test(three_mem_visual_long, 
                                   dv = score,
                                   wid = passport_id, 
                                   within = baseline_number)
...
```{r, include=TRUE}
get_anova_table(three_mem_visual_res)
```

#### Pairwise Comparisons 

```{r, include=FALSE}
three_mem_visual_pwc <- three_mem_visual_long %>% 
  pairwise_t_test(
    score ~ baseline_number, paired = TRUE,
    p.adjust.method = "bonferroni"
  )

three_mem_visual_pwc
```
...
#### Result Visualization
```{r, include=FALSE}
three_mem_visual_pwc <- three_mem_visual_pwc %>% 
  add_xy_position(x = "baseline number")
```

```{r, include=TRUE}
box_p(three_mem_visual_long, baseline_number, score) +
  stat_pvalue_manual(three_mem_visual_pwc) +
  labs(
    subtitle = get_test_label(three_mem_visual_res, detailed = TRUE),
    caption = get_pwc_label(three_mem_visual_pwc)
  )
```

#### Baseline 1 Summary
```{r, include=TRUE}
create_react(three_mem_visual, user_memory_composite_score_visual_1)
```

#### Baseline 1 Histogram 
```{r, include=TRUE}
score_hist(three_mem_visual, user_memory_composite_score_visual_1)
```

#### Baseline 2 Summary
```{r, include=TRUE}
create_react(three_mem_visual, user_memory_composite_score_visual_2)
```

#### Baseline 2 Histogram 
```{r, include=TRUE}
score_hist(three_mem_visual, user_memory_composite_score_visual_2)
```

#### Baseline 3 Summary
```{r, include=TRUE}
create_react(three_mem_visual, user_memory_composite_score_visual_3)
```

#### Baseline 3 Histogram 
```{r, include=TRUE}
score_hist(three_mem_visual, user_memory_composite_score_visual_3)
```
...
```{r, include=FALSE}
four_mem_visual_long <- four_mem_visual %>% 
  pivot_longer(
    cols = c(6:9),
    names_to = "baseline_number",
    names_prefix = "user_memory_composite_score_",
...
```{r, include=TRUE}
box_p(four_mem_visual_long, baseline_number, score)
```
...
```{r, include=FALSE}
four_mem_visual_outliers <- four_mem_visual_long %>% 
  group_by(baseline_number) %>% 
  identify_outliers(score) %>% 
  select(-c(3:6))

four_mem_visual_outliers %>% 
  count(is.outlier)
...
```{r, include=TRUE, fig.width=10}
ggplot(four_mem_visual_long, aes(sample = score)) +
  stat_qq(alpha = 0.3) +
  stat_qq_line() +
  facet_wrap(~baseline_number)
```
...
```{r, include=FALSE}
four_mem_visual_res <- anova_test(four_mem_visual_long, 
                                   dv = score,
                                   wid = passport_id, 
                                   within = baseline_number)
...
```{r, include=TRUE}
get_anova_table(four_mem_visual_res) 
```

#### Pairwise Comparisons 

```{r, include=FALSE}
four_mem_visual_pwc <- four_mem_visual_long %>% 
  pairwise_t_test(
    score ~ baseline_number, paired = TRUE,
    p.adjust.method = "bonferroni"
  )
...
#### Result Visualization
```{r, include=FALSE}
four_mem_visual_pwc <- four_mem_visual_pwc %>% 
  add_xy_position(x = "baseline number")
```

```{r, include=TRUE}
box_p(four_mem_visual_long, baseline_number, score) +
  stat_pvalue_manual(four_mem_visual_pwc) +
  labs(
    subtitle = get_test_label(four_mem_visual_res, detailed = TRUE),
    caption = get_pwc_label(four_mem_visual_pwc)
  )
```

#### Baseline 1 Summary
```{r, include=TRUE}
create_react(four_mem_visual, user_memory_composite_score_visual_1)
```

#### Baseline 1 Histogram 
```{r, include=TRUE}
score_hist(four_mem_visual, user_memory_composite_score_visual_1)
```

#### Baseline 2 Summary
```{r, include=TRUE}
create_react(four_mem_visual, user_memory_composite_score_visual_2)
```

#### Baseline 2 Histogram 
```{r, include=TRUE}
score_hist(four_mem_visual, user_memory_composite_score_visual_2)
```

#### Baseline 3 Summary
```{r, include=TRUE}
create_react(four_mem_visual, user_memory_composite_score_visual_3)
```

#### Baseline 3 Histogram 
```{r, include=TRUE}
score_hist(four_mem_visual, user_memory_composite_score_visual_3)
```

#### Baseline 4 Summary
```{r, include=TRUE}
create_react(four_mem_visual, user_memory_composite_score_visual_4)
```

#### Baseline 4 Histogram 
```{r, include=TRUE}
score_hist(four_mem_visual, user_memory_composite_score_visual_4)
```
...
```{r, include=FALSE}
five_mem_visual_long <- five_mem_visual %>% 
  pivot_longer(
    cols = c(7:11),
    names_to = "baseline_number",
    names_prefix = "user_memory_composite_score_",
...
```{r, include=TRUE}
box_p(five_mem_visual_long, baseline_number, score)
```
...
```{r, include=FALSE}
five_mem_visual_outliers <- five_mem_visual_long %>% 
  group_by(baseline_number) %>% 
  identify_outliers(score) %>% 
  select(-c(3:7))

five_mem_visual_outliers %>% 
  count(is.outlier)
...
```{r, include=TRUE, fig.width=10}
ggplot(five_mem_visual_long, aes(sample = score)) +
  stat_qq(alpha = 0.3) +
  stat_qq_line() +
  facet_wrap(~baseline_number)
```
...
```{r, include=FALSE}
five_mem_visual_res <- anova_test(five_mem_visual_long, 
                                   dv = score,
                                   wid = passport_id, 
                                   within = baseline_number)
...
```{r, include=TRUE}
get_anova_table(five_mem_visual_res)
```

#### Pairwise Comparisons 

```{r, include=FALSE}
five_mem_visual_pwc <- five_mem_visual_long %>% 
  pairwise_t_test(
    score ~ baseline_number, paired = TRUE,
    p.adjust.method = "bonferroni"
  )
...
#### Result Visualization
```{r, include=FALSE}
five_mem_visual_pwc <- five_mem_visual_pwc %>% 
  add_xy_position(x = "baseline number")
```

```{r, include=TRUE}
box_p(five_mem_visual_long, baseline_number, score) +
  stat_pvalue_manual(five_mem_visual_pwc) +
  labs(
    subtitle = get_test_label(five_mem_visual_res, detailed = TRUE),
    caption = get_pwc_label(five_mem_visual_pwc)
  )
```

#### Baseline 1 Summary
```{r, include=TRUE}
create_react(five_mem_visual, user_memory_composite_score_visual_1)
```

#### Baseline 1 Histogram 
```{r, include=TRUE}
score_hist(five_mem_visual, user_memory_composite_score_visual_1)
```

#### Baseline 2 Summary
```{r, include=TRUE}
create_react(five_mem_visual, user_memory_composite_score_visual_2)
```

#### Baseline 2 Histogram 
```{r, include=TRUE}
score_hist(five_mem_visual, user_memory_composite_score_visual_2)
```

#### Baseline 3 Summary
```{r, include=TRUE}
create_react(five_mem_visual, user_memory_composite_score_visual_3)
```

#### Baseline 3 Histogram 
```{r, include=TRUE}
score_hist(five_mem_visual, user_memory_composite_score_visual_3)
```

#### Baseline 4 Summary
```{r, include=TRUE}
create_react(five_mem_visual, user_memory_composite_score_visual_4)
```

#### Baseline 4 Histogram 
```{r, include=TRUE}
score_hist(five_mem_visual, user_memory_composite_score_visual_4)
```

#### Baseline 5 Summary
```{r, include=TRUE}
create_react(five_mem_visual, user_memory_composite_score_visual_5)
```

#### Baseline 5 Histogram 
```{r, include=TRUE}
score_hist(five_mem_visual, user_memory_composite_score_visual_5)
```
...
three_impulse_control

three_impulse_control_long <- three_impulse_control %>% 
  pivot_longer(
    cols = c(5:7),
    names_to = "baseline_number",
    names_prefix = "user_impulse_control_composite_",
...
```{r, include=TRUE}
box_p(three_impulse_control_long, baseline_number, score)
```
...
```{r, include=FALSE}
three_impulse_control_outliers <- three_impulse_control_long %>% 
  group_by(baseline_number) %>% 
  identify_outliers(score) %>% 
  select(-c(3:5))

three_impulse_control_outliers %>% 
  count(is.extreme)
...
```{r, include=TRUE, fig.width=10}
ggplot(three_impulse_control_long, aes(sample = score)) +
  stat_qq(alpha = 0.3) +
  stat_qq_line() +
  facet_wrap(~baseline_number)
```
...
```{r, include=FALSE}
three_impulse_control_res <- anova_test(three_impulse_control_long, 
                                   dv = score,
                                   wid = passport_id, 
                                   within = baseline_number)
...
```{r, include=TRUE}
get_anova_table(three_impulse_control_res)
```

#### Pairwise Comparisons 

```{r, include=FALSE}
three_impulse_control_pwc <- three_impulse_control_long %>% 
  pairwise_t_test(
    score ~ baseline_number, paired = TRUE,
    p.adjust.method = "bonferroni"
  )

three_impulse_control_pwc
```
...
#### Result Visualization
```{r, include=FALSE}
three_impulse_control_pwc <- three_impulse_control_pwc %>% 
  add_xy_position(x = "baseline number")
```

```{r, include=TRUE}
box_p(three_impulse_control_long, baseline_number, score) +
  stat_pvalue_manual(three_impulse_control_pwc) +
  labs(
    subtitle = get_test_label(three_impulse_control_res, detailed = TRUE),
    caption = get_pwc_label(three_impulse_control_pwc)
  )
```

#### Baseline 1 Summary
```{r, include=TRUE}
create_react(three_impulse_control, user_impulse_control_composite_score_1)
```

#### Baseline 1 Histogram 
```{r, include=TRUE}
score_hist(three_impulse_control, user_impulse_control_composite_score_1)
```

#### Baseline 2 Summary
```{r, include=TRUE}
create_react(three_impulse_control, user_impulse_control_composite_score_2)
```

#### Baseline 2 Histogram 
```{r, include=TRUE}
score_hist(three_impulse_control, user_impulse_control_composite_score_2)
```

#### Baseline 3 Summary
```{r, include=TRUE}
create_react(three_impulse_control, user_impulse_control_composite_score_3)
```

#### Baseline 3 Histogram 
```{r, include=TRUE}
score_hist(three_impulse_control, user_impulse_control_composite_score_3)
```
...
```{r, include=FALSE}
four_impulse_control_long <- four_impulse_control %>% 
  pivot_longer(
    cols = c(6:9),
    names_to = "baseline_number",
    names_prefix = "user_impulse_control_composite_",
...
```{r, include=TRUE}
box_p(four_impulse_control_long, baseline_number, score)
```
...
```{r, include=FALSE}
four_impulse_control_outliers <- four_impulse_control_long %>% 
  group_by(baseline_number) %>% 
  identify_outliers(score) %>% 
  select(-c(3:6))

four_impulse_control_outliers %>% 
  count(is.extreme)
...
```{r, include=TRUE, fig.width=10}
ggplot(four_impulse_control_long, aes(sample = score)) +
  stat_qq(alpha = 0.3) +
  stat_qq_line() +
  facet_wrap(~baseline_number)
```
...
```{r, include=FALSE}
four_impulse_control_res <- anova_test(four_impulse_control_long, 
                                   dv = score,
                                   wid = passport_id, 
                                   within = baseline_number)
...
```{r, include=TRUE}
get_anova_table(four_impulse_control_res) 
```

#### Pairwise Comparisons 

```{r, include=FALSE}
four_impulse_control_pwc <- four_impulse_control_long %>% 
  pairwise_t_test(
    score ~ baseline_number, paired = TRUE,
    p.adjust.method = "bonferroni"
  )

four_impulse_control_pwc
```
...
#### Result Visualization
```{r, include=FALSE}
four_impulse_control_pwc <- four_impulse_control_pwc %>% 
  add_xy_position(x = "baseline number")
```

```{r, include=TRUE}
box_p(four_impulse_control_long, baseline_number, score) +
  stat_pvalue_manual(four_impulse_control_pwc) +
  labs(
    subtitle = get_test_label(four_impulse_control_res, detailed = TRUE),
    caption = get_pwc_label(four_impulse_control_pwc)
  )
```

#### Baseline 1 Summary
```{r, include=TRUE}
create_react(four_impulse_control, user_impulse_control_composite_score_1)
```

#### Baseline 1 Histogram 
```{r, include=TRUE}
score_hist(four_impulse_control, user_impulse_control_composite_score_1)
```

#### Baseline 2 Summary
```{r, include=TRUE}
create_react(four_impulse_control, user_impulse_control_composite_score_2)
```

#### Baseline 2 Histogram 
```{r, include=TRUE}
score_hist(four_impulse_control, user_impulse_control_composite_score_2)
```

#### Baseline 3 Summary
```{r, include=TRUE}
create_react(four_impulse_control, user_impulse_control_composite_score_3)
```

#### Baseline 3 Histogram 
```{r, include=TRUE}
score_hist(four_impulse_control, user_impulse_control_composite_score_3)
```

#### Baseline 4 Summary
```{r, include=TRUE}
create_react(four_impulse_control, user_impulse_control_composite_score_4)
```

#### Baseline 4 Histogram 
```{r, include=TRUE}
score_hist(four_impulse_control, user_impulse_control_composite_score_4)
```
...
```{r, include=FALSE}
five_impulse_control_long <- five_impulse_control %>% 
  pivot_longer(
    cols = c(7:11),
    names_to = "baseline_number",
    names_prefix = "user_impulse_control_composite_",
...
```{r, include=TRUE}
box_p(five_impulse_control_long, baseline_number, score)
```
...
```{r, include=FALSE}
five_impulse_control_outliers <- five_impulse_control_long %>% 
  group_by(baseline_number) %>% 
  identify_outliers(score) %>% 
  select(-c(3:7))

five_impulse_control_outliers %>% 
  count(is.outlier)
...
```{r, include=TRUE, fig.width=10}
ggplot(five_impulse_control_long, aes(sample = score)) +
  stat_qq(alpha = 0.3) +
  stat_qq_line() +
  facet_wrap(~baseline_number)
```
...
```{r, include=FALSE}
five_impulse_control_res <- anova_test(five_impulse_control_long, 
                                   dv = score,
                                   wid = passport_id, 
                                   within = baseline_number)
...
```{r, include=TRUE}
get_anova_table(five_impulse_control_res)
```

#### Pairwise Comparisons 

```{r, include=FALSE}
five_impulse_control_pwc <- five_impulse_control_long %>% 
  pairwise_t_test(
    score ~ baseline_number, paired = TRUE,
    p.adjust.method = "bonferroni"
  )

five_impulse_control_pwc
```
...
#### Result Visualization
```{r, include=FALSE}
five_impulse_control_pwc <- five_impulse_control_pwc %>% 
  add_xy_position(x = "baseline number")
```

```{r, include=TRUE}
box_p(five_impulse_control_long, baseline_number, score) +
  stat_pvalue_manual(five_impulse_control_pwc) +
  labs(
    subtitle = get_test_label(five_impulse_control_res, detailed = TRUE),
    caption = get_pwc_label(five_impulse_control_pwc)
  )
```

#### Baseline 1 Summary
```{r, include=TRUE}
create_react(five_impulse_control, user_impulse_control_composite_score_1)
```

#### Baseline 1 Histogram 
```{r, include=TRUE}
score_hist(five_impulse_control, user_impulse_control_composite_score_1)
```

#### Baseline 2 Summary
```{r, include=TRUE}
create_react(five_impulse_control, user_impulse_control_composite_score_2)
```

#### Baseline 2 Histogram 
```{r, include=TRUE}
score_hist(five_impulse_control, user_impulse_control_composite_score_2)
```

#### Baseline 3 Summary
```{r, include=TRUE}
create_react(five_impulse_control, user_impulse_control_composite_score_3)
```

#### Baseline 3 Histogram 
```{r, include=TRUE}
score_hist(five_impulse_control, user_impulse_control_composite_score_3)
```

#### Baseline 4 Summary
```{r, include=TRUE}
create_react(five_impulse_control, user_impulse_control_composite_score_4)
```

#### Baseline 4 Histogram 
```{r, include=TRUE}
score_hist(five_impulse_control, user_impulse_control_composite_score_4)
```

#### Baseline 5 Summary
```{r, include=TRUE}
create_react(five_impulse_control, user_impulse_control_composite_score_5)
```

#### Baseline 5 Histogram 
```{r, include=TRUE}
score_hist(five_impulse_control, user_impulse_control_composite_score_5)
```
...
three_reaction_time_long <- three_reaction_time %>% 
  pivot_longer(
    cols = c(5:7),
    names_to = "baseline_number",
    names_prefix = "user_reaction_time_composite_",
...
```{r, include=TRUE}
box_p(three_reaction_time_long, baseline_number, score)
```
...
```{r, include=FALSE}
three_reaction_time_outliers <- three_reaction_time_long %>% 
  group_by(baseline_number) %>% 
  identify_outliers(score) %>% 
  select(-c(3:5))

three_reaction_time_outliers %>% 
  count(is.outlier)
...
```{r, include=TRUE, fig.width=10}
ggplot(three_reaction_time_long, aes(sample = score)) +
  stat_qq(alpha = 0.3) +
  stat_qq_line() +
  facet_wrap(~baseline_number)
```
...
```{r, include=FALSE}
three_reaction_time_res <- anova_test(three_reaction_time_long, 
                                   dv = score,
                                   wid = passport_id, 
                                   within = baseline_number)
...
```{r, include=TRUE}
get_anova_table(three_reaction_time_res)
```

#### Pairwise Comparisons 

```{r, include=FALSE}
three_reaction_time_pwc <- three_reaction_time_long %>% 
  pairwise_t_test(
    score ~ baseline_number, paired = TRUE,
    p.adjust.method = "bonferroni"
  )

three_reaction_time_pwc
```
...
#### Result Visualization
```{r, include=FALSE}
three_reaction_time_pwc <- three_reaction_time_pwc %>% 
  add_xy_position(x = "baseline number")
```

```{r, include=TRUE}
box_p(three_reaction_time_long, baseline_number, score) +
  stat_pvalue_manual(three_reaction_time_pwc) +
  labs(
    subtitle = get_test_label(three_reaction_time_res, detailed = TRUE),
    caption = get_pwc_label(three_reaction_time_pwc)
  )
```

#### Baseline 1 Summary
```{r, include=TRUE}
create_react(three_reaction_time, user_reaction_time_composite_score_1)
```

#### Baseline 1 Histogram 
```{r, include=TRUE}
score_hist(three_reaction_time, user_reaction_time_composite_score_1)
```

#### Baseline 2 Summary
```{r, include=TRUE}
create_react(three_reaction_time, user_reaction_time_composite_score_2)
```

#### Baseline 2 Histogram 
```{r, include=TRUE}
score_hist(three_reaction_time, user_reaction_time_composite_score_2)
```

#### Baseline 3 Summary
```{r, include=TRUE}
create_react(three_reaction_time, user_reaction_time_composite_score_3)
```

#### Baseline 3 Histogram 
```{r, include=TRUE}
score_hist(three_reaction_time, user_reaction_time_composite_score_3)
```
...
```{r, include=FALSE}
four_reaction_time_long <- four_reaction_time %>% 
  pivot_longer(
    cols = c(6:9),
    names_to = "baseline_number",
    names_prefix = "user_reaction_time_composite_",
...
```{r, include=TRUE}
box_p(four_reaction_time_long, baseline_number, score)
```
...
```{r, include=FALSE}
four_reaction_time_outliers <- four_reaction_time_long %>% 
  group_by(baseline_number) %>% 
  identify_outliers(score) %>% 
  select(-c(3:6))

four_reaction_time_outliers %>% 
  count(is.outlier)
...
```{r, include=TRUE, fig.width=10}
ggplot(four_reaction_time_long, aes(sample = score)) +
  stat_qq(alpha = 0.3) +
  stat_qq_line() +
  facet_wrap(~baseline_number)
```
...
```{r, include=FALSE}
four_reaction_time_res <- anova_test(four_reaction_time_long, 
                                   dv = score,
                                   wid = passport_id, 
                                   within = baseline_number)
...
```{r, include=TRUE}
get_anova_table(four_reaction_time_res) 
```

#### Pairwise Comparisons 

```{r, include=FALSE}
four_reaction_time_pwc <- four_reaction_time_long %>% 
  pairwise_t_test(
    score ~ baseline_number, paired = TRUE,
    p.adjust.method = "bonferroni"
  )

four_reaction_time_pwc
```
...
#### Result Visualization
```{r, include=FALSE}
four_reaction_time_pwc <- four_reaction_time_pwc %>% 
  add_xy_position(x = "baseline number")
```

```{r, include=TRUE}
box_p(four_reaction_time_long, baseline_number, score) +
  stat_pvalue_manual(four_reaction_time_pwc) +
  labs(
    subtitle = get_test_label(four_reaction_time_res, detailed = TRUE),
    caption = get_pwc_label(four_reaction_time_pwc)
  )
```

#### Baseline 1 Summary
```{r, include=TRUE}
create_react(four_reaction_time, user_reaction_time_composite_score_1)
```

#### Baseline 1 Histogram 
```{r, include=TRUE}
score_hist(four_reaction_time, user_reaction_time_composite_score_1)
```

#### Baseline 2 Summary
```{r, include=TRUE}
create_react(four_reaction_time, user_reaction_time_composite_score_2)
```

#### Baseline 2 Histogram 
```{r, include=TRUE}
score_hist(four_reaction_time, user_reaction_time_composite_score_2)
```

#### Baseline 3 Summary
```{r, include=TRUE}
create_react(four_reaction_time, user_reaction_time_composite_score_3)
```

#### Baseline 3 Histogram 
```{r, include=TRUE}
score_hist(four_reaction_time, user_reaction_time_composite_score_3)
```

#### Baseline 4 Summary
```{r, include=TRUE}
create_react(four_reaction_time, user_reaction_time_composite_score_4)
```

#### Baseline 4 Histogram 
```{r, include=TRUE}
score_hist(four_reaction_time, user_reaction_time_composite_score_4)
```
...
```{r, include=FALSE}
five_reaction_time_long <- five_reaction_time %>% 
  pivot_longer(
    cols = c(7:11),
    names_to = "baseline_number",
    names_prefix = "user_reaction_time_composite_",
...
```{r, include=TRUE}
box_p(five_reaction_time_long, baseline_number, score)
```
...
```{r, include=FALSE}
five_reaction_time_outliers <- five_reaction_time_long %>% 
  group_by(baseline_number) %>% 
  identify_outliers(score) %>% 
  select(-c(3:7))

five_reaction_time_outliers %>% 
  count(is.outlier)
...
```{r, include=TRUE, fig.width=10}
ggplot(five_reaction_time_long, aes(sample = score)) +
  stat_qq(alpha = 0.3) +
  stat_qq_line() +
  facet_wrap(~baseline_number)
```
...
```{r, include=FALSE}
five_reaction_time_res <- anova_test(five_reaction_time_long, 
                                   dv = score,
                                   wid = passport_id, 
                                   within = baseline_number)
...
```{r, include=TRUE}
get_anova_table(five_reaction_time_res)
```

#### Pairwise Comparisons 

```{r, include=FALSE}
five_reaction_time_pwc <- five_reaction_time_long %>% 
  pairwise_t_test(
    score ~ baseline_number, paired = TRUE,
    p.adjust.method = "bonferroni"
  )

five_reaction_time_pwc
```
...
#### Result Visualization
```{r, include=FALSE}
five_reaction_time_pwc <- five_reaction_time_pwc %>% 
  add_xy_position(x = "baseline number")
```

```{r, include=TRUE}
box_p(five_reaction_time_long, baseline_number, score) +
  stat_pvalue_manual(five_reaction_time_pwc) +
  labs(
    subtitle = get_test_label(five_reaction_time_res, detailed = TRUE),
    caption = get_pwc_label(five_reaction_time_pwc)
  )
```

#### Baseline 1 Summary
```{r, include=TRUE}
create_react(five_reaction_time, user_reaction_time_composite_score_1)
```

#### Baseline 1 Histogram 
```{r, include=TRUE}
score_hist(five_reaction_time, user_reaction_time_composite_score_1)
```

#### Baseline 2 Summary
```{r, include=TRUE}
create_react(five_reaction_time, user_reaction_time_composite_score_2)
```

#### Baseline 2 Histogram 
```{r, include=TRUE}
score_hist(five_reaction_time, user_reaction_time_composite_score_2)
```

#### Baseline 3 Summary
```{r, include=TRUE}
create_react(five_reaction_time, user_reaction_time_composite_score_3)
```

#### Baseline 3 Histogram 
```{r, include=TRUE}
score_hist(five_reaction_time, user_reaction_time_composite_score_3)
```

#### Baseline 4 Summary
```{r, include=TRUE}
create_react(five_reaction_time, user_reaction_time_composite_score_4)
```

#### Baseline 4 Histogram 
```{r, include=TRUE}
score_hist(five_reaction_time, user_reaction_time_composite_score_4)
```

#### Baseline 5 Summary
```{r, include=TRUE}
create_react(five_reaction_time, user_reaction_time_composite_score_5)
```

#### Baseline 5 Histogram 
```{r, include=TRUE}
score_hist(five_reaction_time, user_reaction_time_composite_score_5)
```
...
three_visual_motor_long <- three_visual_motor %>% 
  pivot_longer(
    cols = c(5:7),
    names_to = "baseline_number",
    names_prefix = "user_visual_motor_composite_",
...
```{r, include=TRUE}
box_p(three_visual_motor_long, baseline_number, score)
```
...
```{r, include=FALSE}
three_visual_motor_outliers <- three_visual_motor_long %>% 
  group_by(baseline_number) %>% 
  identify_outliers(score) %>% 
  select(-c(3:5))

three_visual_motor_outliers %>% 
  count(is.outlier)
...
```{r, include=TRUE, fig.width=10}
ggplot(three_visual_motor_long, aes(sample = score)) +
  stat_qq(alpha = 0.3) +
  stat_qq_line() +
  facet_wrap(~baseline_number)
```
...
```{r, include=FALSE}
three_visual_motor_res <- anova_test(three_visual_motor_long, 
                                   dv = score,
                                   wid = passport_id, 
                                   within = baseline_number)
...
```{r, include=TRUE}
get_anova_table(three_visual_motor_res)
```

#### Pairwise Comparisons 

```{r, include=FALSE}
three_visual_motor_pwc <- three_visual_motor_long %>% 
  pairwise_t_test(
    score ~ baseline_number, paired = TRUE,
    p.adjust.method = "bonferroni"
  )

three_visual_motor_pwc
```
...
#### Result Visualization
```{r, include=FALSE}
three_visual_motor_pwc <- three_visual_motor_pwc %>% 
  add_xy_position(x = "baseline number")
```

```{r, include=TRUE}
box_p(three_visual_motor_long, baseline_number, score) +
  stat_pvalue_manual(three_visual_motor_pwc) +
  labs(
    subtitle = get_test_label(three_visual_motor_res, detailed = TRUE),
    caption = get_pwc_label(three_visual_motor_pwc)
  )
```

#### Baseline 1 Summary
```{r, include=TRUE}
create_react(three_visual_motor, user_visual_motor_composite_score_1)
```

#### Baseline 1 Histogram 
```{r, include=TRUE}
score_hist(three_visual_motor, user_visual_motor_composite_score_1)
```

#### Baseline 2 Summary
```{r, include=TRUE}
create_react(three_visual_motor, user_visual_motor_composite_score_2)
```

#### Baseline 2 Histogram 
```{r, include=TRUE}
score_hist(three_visual_motor, user_visual_motor_composite_score_2)
```

#### Baseline 3 Summary
```{r, include=TRUE}
create_react(three_visual_motor, user_visual_motor_composite_score_3)
```

#### Baseline 3 Histogram 
```{r, include=TRUE}
score_hist(three_visual_motor, user_visual_motor_composite_score_3)
```
...
```{r, include=FALSE}
four_visual_motor_long <- four_visual_motor %>% 
  pivot_longer(
    cols = c(6:9),
    names_to = "baseline_number",
    names_prefix = "user_visual_motor_composite_",
...
```{r, include=TRUE}
box_p(four_visual_motor_long, baseline_number, score)
```
...
```{r, include=FALSE}
four_visual_motor_outliers <- four_visual_motor_long %>% 
  group_by(baseline_number) %>% 
  identify_outliers(score) %>% 
  select(-c(3:6))

four_visual_motor_outliers %>% 
  count(is.outlier)
...
```{r, include=TRUE, fig.width=10}
ggplot(four_visual_motor_long, aes(sample = score)) +
  stat_qq(alpha = 0.3) +
  stat_qq_line() +
  facet_wrap(~baseline_number)
```
...
```{r, include=FALSE}
four_visual_motor_res <- anova_test(four_visual_motor_long, 
                                   dv = score,
                                   wid = passport_id, 
                                   within = baseline_number)
...
```{r, include=TRUE}
get_anova_table(four_visual_motor_res) 
```

#### Pairwise Comparisons 

```{r, include=FALSE}
four_visual_motor_pwc <- four_visual_motor_long %>% 
  pairwise_t_test(
    score ~ baseline_number, paired = TRUE,
    p.adjust.method = "bonferroni"
  )

four_visual_motor_pwc
```
...
#### Result Visualization
```{r, include=FALSE}
four_visual_motor_pwc <- four_visual_motor_pwc %>% 
  add_xy_position(x = "baseline number")
```

```{r, include=TRUE}
box_p(four_visual_motor_long, baseline_number, score) +
  stat_pvalue_manual(four_visual_motor_pwc) +
  labs(
    subtitle = get_test_label(four_visual_motor_res, detailed = TRUE),
    caption = get_pwc_label(four_visual_motor_pwc)
  )
```

#### Baseline 1 Summary
```{r, include=TRUE}
create_react(four_visual_motor, user_visual_motor_composite_score_1)
```

#### Baseline 1 Histogram 
```{r, include=TRUE}
score_hist(four_visual_motor, user_visual_motor_composite_score_1)
```

#### Baseline 2 Summary
```{r, include=TRUE}
create_react(four_visual_motor, user_visual_motor_composite_score_2)
```

#### Baseline 2 Histogram 
```{r, include=TRUE}
score_hist(four_visual_motor, user_visual_motor_composite_score_2)
```

#### Baseline 3 Summary
```{r, include=TRUE}
create_react(four_visual_motor, user_visual_motor_composite_score_3)
```

#### Baseline 3 Histogram 
```{r, include=TRUE}
score_hist(four_visual_motor, user_visual_motor_composite_score_3)
```

#### Baseline 4 Summary
```{r, include=TRUE}
create_react(four_visual_motor, user_visual_motor_composite_score_4)
```

#### Baseline 4 Histogram 
```{r, include=TRUE}
score_hist(four_visual_motor, user_visual_motor_composite_score_4)
```
...
```{r, include=FALSE}
five_visual_motor_long <- five_visual_motor %>% 
  pivot_longer(
    cols = c(7:11),
    names_to = "baseline_number",
    names_prefix = "user_visual_motor_composite_",
...
```{r, include=TRUE}
box_p(five_visual_motor_long, baseline_number, score)
```
...
```{r, include=FALSE}
five_visual_motor_outliers <- five_visual_motor_long %>% 
  group_by(baseline_number) %>% 
  identify_outliers(score) %>% 
  select(-c(3:7))

five_visual_motor_outliers %>% 
  count(is.outlier)
...
```{r, include=TRUE, fig.width=10}
ggplot(five_visual_motor_long, aes(sample = score)) +
  stat_qq(alpha = 0.3) +
  stat_qq_line() +
  facet_wrap(~baseline_number)
```
...
```{r, include=FALSE}
five_visual_motor_res <- anova_test(five_visual_motor_long, 
                                   dv = score,
                                   wid = passport_id, 
                                   within = baseline_number)
...
```{r, include=TRUE}
get_anova_table(five_visual_motor_res)
```

#### Pairwise Comparisons 

```{r, include=FALSE}
five_visual_motor_pwc <- five_visual_motor_long %>% 
  pairwise_t_test(
    score ~ baseline_number, paired = TRUE,
    p.adjust.method = "bonferroni"
  )

five_visual_motor_pwc
```
...
#### Result Visualization
```{r, include=FALSE}
five_visual_motor_pwc <- five_visual_motor_pwc %>% 
  add_xy_position(x = "baseline number")
```

```{r, include=TRUE}
box_p(five_visual_motor_long, baseline_number, score) +
  stat_pvalue_manual(five_visual_motor_pwc) +
  labs(
    subtitle = get_test_label(five_visual_motor_res, detailed = TRUE),
    caption = get_pwc_label(five_visual_motor_pwc)
  )
```

#### Baseline 1 Summary
```{r, include=TRUE}
create_react(five_visual_motor, user_visual_motor_composite_score_1)
```

#### Baseline 1 Histogram 
```{r, include=TRUE}
score_hist(five_visual_motor, user_visual_motor_composite_score_1)
```

#### Baseline 2 Summary
```{r, include=TRUE}
create_react(five_visual_motor, user_visual_motor_composite_score_2)
```

#### Baseline 2 Histogram 
```{r, include=TRUE}
score_hist(five_visual_motor, user_visual_motor_composite_score_2)
```

#### Baseline 3 Summary
```{r, include=TRUE}
create_react(five_visual_motor, user_visual_motor_composite_score_3)
```

#### Baseline 3 Histogram 
```{r, include=TRUE}
score_hist(five_visual_motor, user_visual_motor_composite_score_3)
```

#### Baseline 4 Summary
```{r, include=TRUE}
create_react(five_visual_motor, user_visual_motor_composite_score_4)
```

#### Baseline 4 Histogram 
```{r, include=TRUE}
score_hist(five_visual_motor, user_visual_motor_composite_score_4)
```

#### Baseline 5 Summary
```{r, include=TRUE}
create_react(five_visual_motor, user_visual_motor_composite_score_5)
```

#### Baseline 5 Histogram 
```{r, include=TRUE}
score_hist(five_visual_motor, user_visual_motor_composite_score_5)
```
